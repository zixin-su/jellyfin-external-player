...
3. The app should provide a settings page for:
   ...
   - Jellyfin stream preference switch.
   - STRM/local path behavior.
...
6. The preferred playback target should be the Jellyfin HTTP stream URL, so external players can stream through Jellyfin.
7. If direct file access is desired, optional path mapping should still be available.
8. Poster center play buttons and detail page play buttons should be intercepted.
9. Non-play actions must keep their Jellyfin behavior:
   - Favorite.
   - Watched status.
   - More menu.
   - Detail page toolbar actions other than play.
10. Series and season poster play buttons should follow Jellyfin-style behavior:
...
11. Login state and settings should migrate between packaged builds when the app folder is copied or updated.
12. Zoom shortcuts should work consistently:
    - `Ctrl+-` zoom out.
    - `Ctrl+=` / `Ctrl++` zoom in.
    - `Ctrl+0` reset zoom.
...
- Require direct client access to NAS paths.
...
- By default, Jellyfin HTTP stream URLs are preferred over direct file paths.
